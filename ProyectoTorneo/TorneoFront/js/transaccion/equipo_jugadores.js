  ...
  .addEventListener("click", async () => {
    ...
    rows.forEach((row) => {
      ...
      const jugador = {
        ...
        idPosicion:
          cells[6].innerText === "Arquero"
            ? 1
            : cells[6].innerText === "Defensa"
            ? 2
            : cells[6].innerText === "Centrocampista"
            ? 3
            : cells[6].innerText === "Delantero"
            ? 4
            : 0,
        rol:
          cells[7].innerText === "" || parseInt(cells[7].innerText) === 0
            ? null
            : parseInt(cells[7].innerText), // Asegúrate de que el rol es un número
      };
...
function cargarEquiposEnFormulario(equipo) {
  ...
  document.getElementById("fechaFundacion").value =
    equipo.fechaFundacion.substring(0, 10);
}
